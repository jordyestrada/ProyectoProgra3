-- =========================================================
-- SCRIPT COMPLETO - Reservas Municipales (COMPATIBLE)
-- Versión compatible con PostgreSQL más antiguas
...
-- =========================
-- FOREIGN KEYS (SIN IF NOT EXISTS)
-- =========================

BEGIN;

-- Primero eliminamos constraints existentes si existen (para evitar duplicados)
DO $$ 
BEGIN
    -- Usuarios
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_app_user_role') THEN
        ALTER TABLE app_user DROP CONSTRAINT fk_app_user_role;
    END IF;
    
    -- Espacios
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_type') THEN
        ALTER TABLE space DROP CONSTRAINT fk_space_type;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_feature_space') THEN
        ALTER TABLE space_feature DROP CONSTRAINT fk_space_feature_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_feature_feature') THEN
        ALTER TABLE space_feature DROP CONSTRAINT fk_space_feature_feature;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_image_space') THEN
        ALTER TABLE space_image DROP CONSTRAINT fk_space_image_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_schedule_space') THEN
        ALTER TABLE space_schedule DROP CONSTRAINT fk_space_schedule_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_closure_space') THEN
        ALTER TABLE space_closure DROP CONSTRAINT fk_space_closure_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_space_rate_space') THEN
        ALTER TABLE space_rate DROP CONSTRAINT fk_space_rate_space;
    END IF;
    
    -- Reservas
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_reservation_space') THEN
        ALTER TABLE reservation DROP CONSTRAINT fk_reservation_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_reservation_user') THEN
        ALTER TABLE reservation DROP CONSTRAINT fk_reservation_user;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_reservation_rate') THEN
        ALTER TABLE reservation DROP CONSTRAINT fk_reservation_rate;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_reservation_confirmed_by_user') THEN
        ALTER TABLE reservation DROP CONSTRAINT fk_reservation_confirmed_by_user;
    END IF;
    
    -- Reseñas
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_review_space') THEN
        ALTER TABLE review DROP CONSTRAINT fk_review_space;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_review_user') THEN
        ALTER TABLE review DROP CONSTRAINT fk_review_user;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_review_reservation') THEN
        ALTER TABLE review DROP CONSTRAINT fk_review_reservation;
    END IF;
END $$;

-- Ahora agregamos las constraints
-- Usuarios
...
ALTER TABLE space_image
  ADD CONSTRAINT fk_space_image_space
  FOREIGN KEY (space_id) REFERENCES space(space_id) ON DELETE CASCADE;
...
ALTER TABLE reservation
  ADD CONSTRAINT fk_reservation_confirmed_by_user
